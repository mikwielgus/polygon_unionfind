...
use maplike::ops::{Clear, Get, Insert, Remove, Set};
...
impl<K: RTreeObject> Keyed for AsRefRTree<K> {
    type Key = K;
}
...
impl<K: RTreeObject + PartialEq> Clear for AsRefRTree<K> {
    #[inline(always)]
    fn clear(&mut self) {
        Clear::clear(&mut self.rtree);
    }
}
